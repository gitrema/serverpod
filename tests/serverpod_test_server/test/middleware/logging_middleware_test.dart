...
void main() {
  group('loggingMiddleware - basic logging', () {
    ...
    test(
        'Given basic logging middleware, When a request is made, Then it logs method, URI, status code, and timing',
        () async {
      // Make request
      ...
      expect(logMessage, matches(RegExp(r'\(\d+ms\)')),
          reason: 'Should include timing in ms');
    });

    test(
        'Given basic logging middleware, When multiple requests are made, Then each request is logged independently',
        () async {
    ...
    test(
        'Given basic logging middleware, When a request completes, Then it logs the status code correctly',
        () async {
    ...
    test(
        'Given basic logging middleware, When a request is made, Then it logs timestamp in UTC format',
        () async {
    ...
    test(
        'Given basic logging middleware, When a request is made, Then it tracks timing accurately',
        () async {
      await httpClient.post(
        endpoint,
        headers: {'Content-Type': 'application/json'},
  ...
  group('loggingMiddleware - verbose mode', () {
    ...
    test(
        'Given verbose logging middleware, When a request is made, Then it logs request and response headers',
        () async {
      await httpClient.post(
        endpoint,
        headers: {'Content-Type': 'application/json'},
    ...
    test(
        'Given verbose logging middleware, When a request is made, Then response line includes timing',
        () async {
      await httpClient.post(
        endpoint,
        headers: {'Content-Type': 'application/json'},
  ...
  group('loggingMiddleware - multiple middleware interaction', () {
    ...
    test(
        'Given logging middleware in a chain, When a request is made, Then it works correctly with other middleware',
        () async {
      await httpClient.post(
        endpoint,
        headers: {'Content-Type': 'application/json'},
  ...
  group('loggingMiddleware - error handling', () {
    ...
    test(
        'Given logging middleware with custom logger, When an error occurs, Then it logs error with request details',
        () async {
    ...
    test(
        'Given logging middleware, When an error occurs, Then it includes stack trace in logs',
        () async {
      try {
        await httpClient.post(
          endpoint,
  ...
  group('loggingMiddleware - separate error logger', () {
    ...
    test(
        'Given logging middleware with separate error logger, When an error occurs, Then it routes errors to errorLogger',
        () async {
      // Make request that will trigger error
      try {
        await httpClient.post(
  ...
  group('loggingMiddleware - no server required', () {
    test(
        'Given loggingMiddleware factory, When called, Then it creates valid Middleware',
        () {
    ...
    test(
        'Given loggingMiddleware factory, When called with custom logger, Then it accepts the logger',
        () {
    ...
    test(
        'Given loggingMiddleware factory, When called with verbose parameter, Then it accepts the parameter',
        () {

...
void main() {
  group('Core middleware ordering', () {
    late Serverpod pod;
    late int port;
    ...
    group('Given user middleware that returns early without calling next', () {
      ...
      test(
          'When request is made Then core _headers middleware still applies CORS headers',
          () async {
    ...
    group('Given user middleware that throws an exception', () {
      ...
      test(
          'When request is made Then core _reportException middleware catches it and returns 500 error',
          () async {
        final response = await http.get(
          Uri.http('localhost:$port', '/'),
        );
      ...
      test(
          'When request is made Then response still has CORS headers from _headers middleware',
          () async {
        final response = await http.get(
          Uri.http('localhost:$port', '/'),
        );
    ...
    group('Given user middleware that throws SerializableException', () {
      setUp(() async {
        ...
        pod = Serverpod(
          ...
          experimentalFeatures: ExperimentalFeatures(
            middleware: [serializableExceptionMiddleware],
          ),
        );
      ...
      test(
          'When request is made Then core _reportException middleware converts it to 400 bad request',
          () async {
        final response = await http.get(
          Uri.http('localhost:$port', '/'),
        );
    ...
    group('Given multiple user middleware returning early', () {
      ...
      test(
          'When requests are made Then all early returns still have core headers applied',
          () async {
